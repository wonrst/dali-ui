...
void LabelImpl::SetTextReveal(const Ui::Text::Reveal& reveal)
{
  ...
  const float                      authoredBlurRadius           = enabled ? reveal.GetBlurRadius() : 0.0f;
  const float                      authoredBlurDurationRatio    = enabled ? reveal.GetBlurDurationRatio() : 1.0f;
  const auto                       authoredBlurQuality          = enabled ? reveal.GetBlurQuality() : Ui::Text::Reveal::BlurQuality::PERFORMANCE;
  ...
  if((!data && !enabled) ||
     (data && data->enabled == enabled &&
      (!enabled || (data->unit == authoredUnit &&
                    ...
                    data->blurQuality == authoredBlurQuality &&
...
Ui::Text::Reveal LabelImpl::GetTextReveal() const
{
  ...
  reveal.SetBlurRadius(data->blurRadius);
  reveal.SetBlurDurationRatio(data->blurDurationRatio);
  reveal.SetBlurQuality(data->blurQuality);
  return reveal;
}
