...
class TextExperimental2Controller : public ConnectionTracker
{
public:
  explicit TextExperimental2Controller(Application& application)
  : mApplication(application),
    mRandomEngine(std::random_device{}())
  {
    mApplication.InitSignal().Connect(this, &TextExperimental2Controller::OnInit);
...
private: // ---------- style / option helpers ----------
  ...
  const char* GetAsyncName() const
  {
    return mTilesAsyncRendering ? "ASYNC" : "SYNC";
  }
...
private: // ---------- UI ----------
  ...
  {
    mMarkupColorEnabled = enabled;

    for(auto& tile : mTiles)
    {
      if(tile.label)
      {
        tile.label.SetMarkupEnabled(enabled);
      }
    }
  ...
  void UpdateBallCountLabel()
  {
    ...
    mBallCountLabel.SetText(text.c_str());
  }
  ...
  void ResetColorAnimationTextColors()
  {
    if(mBallCountLabel)
    {
      mBallCountLabel.SetTextColor(UiColor(COLOR_INFO));
    }
    ...
    for(auto& tile : mTiles)
    {
      if(tile.label)
      {
        tile.label.SetTextColor(UiColor(COLOR_TEXT));
      }
    }
  }
  ...
  void SetTileLabelLayout(Tile& tile)
  {
    tile.label.SetLayoutParams(
  ...
  void RecreateTileLabel(Tile& tile)
  {
    if(tile.label)
    {
      mTileRoot.Remove(tile.label);
    }

    tile.label = CreateTileLabel(tile.glyphCols, tile.glyphRows);
    SetTileLabelLayout(tile);
    mTileRoot.Add(tile.label);
  }
  ...
  void AttachDaliColorAnimations()
  {
    ...
    {
      PlayColorAnimation(tile.label);
    }
  }

  void RequestDaliColorAnimationStart()
  {
    PrepareDaliColorAnimationText();
    mPendingDaliColorAnimationStart = true;
  }

  void SetDaliColorAnimationEnabled(bool enabled)
  {
    if(enabled == mDaliColorAnimationEnabled)
    {
      return;
    }

    mDaliColorAnimationEnabled = enabled;

    if(mDaliColorAnimationEnabled)
    {
      SetMarkupColorEnabled(false);
      RequestDaliColorAnimationStart();
    }
    else
    {
      mPendingDaliColorAnimationStart = false;
  ...
  Label CreateTileLabel(uint32_t glyphCols, uint32_t glyphRows)
  {
    Label label = Label::New("");
    label
      .SetRequestedWidth(static_cast<float>(glyphCols) * GLYPH_WIDTH)
      .SetRequestedHeight(static_cast<float>(glyphRows) * GLYPH_HEIGHT)
      .SetFontSize(16.0f)
      .SetMarkupEnabled(mMarkupColorEnabled)
      .SetTextColor(UiColor(COLOR_TEXT))
      .SetBackgroundColor(GetClearTileColor())
      .SetFontFamily("Ubuntu Mono")
      .SetMultiLine(true)
      .SetOverflowMode(Text::OverflowMode::CLIP)
      .SetBorderlineWidth(GRID_WIDTH)
      .SetBorderlineColor(GetGridColor())
      .SetAsyncRendering(mTilesAsyncRendering);
  ...
  View CreateContents()
  {
    ...
    mBallCountLabel = Label::New("");
    mBallCountLabel
      .SetFontSize(12.0f)
      .SetTextColor(UiColor(COLOR_INFO));
    ...
    return StackLayout::New(StackOrientation::VERTICAL)
      ...
      .Children({
        Label::New("Text Experimental2 Example")
  ...
  void RebuildTiles(uint32_t cols, uint32_t rows)
  {
    ...
    const bool restartColorAnimation = mDaliColorAnimationEnabled;
    if(restartColorAnimation)
    {
      StopDaliColorAnimations(false);
    }
    ...
    for(uint32_t tileY = 0u; tileY < mTileCountY; ++tileY)
    {
      for(uint32_t tileX = 0u; tileX < mTileCountX; ++tileX)
      {
        ...
        tile.label.SetText(tile.previousText.c_str());
  ...
  {
    mTilesAsyncRendering = enabled;

    for(auto& tile : mTiles)
    {
      tile.label.SetAsyncRendering(enabled);
    }
  ...
  void SetDirtyTileDebugVisible(bool visible)
  {
    ...
    if(!mDirtyTileDebugVisible)
    {
      ...
      for(auto& tile : mTiles)
      {
        if(tile.label)
        {
          tile.label.SetBackgroundColor(clearColor);
        }
      }
    }
  }
  ...
  void SetGridVisible(bool visible)
  {
    ...
    for(auto& tile : mTiles)
    {
      if(tile.label)
      {
        tile.label.SetBorderlineWidth(GRID_WIDTH);
        tile.label.SetBorderlineColor(color);
      }
    }
  }
...
private: // ---------- tile presentation ----------
  ...
  bool RebuildDirtyTileTextPlain(Tile& tile)
  {
    ...
    {
      tile.label.SetText(tile.workingText.c_str());
  ...
  void ApplyTileDebugBackground(Tile& tile, bool changed)
  {
    if(mDirtyTileDebugVisible)
    {
      tile.label.SetBackgroundColor(changed ? GetDirtyTileColor() : GetClearTileColor());
    }
  }
...
private: // ---------- frame pipeline ----------
  ...
  void PrepareFrame(uint32_t cols, uint32_t rows)
  {
...
private: // ---------- app lifecycle ----------
  void OnInit(Application& application)
  {
    ...
    mBallTimer.TickSignal().Connect(this, &TextExperimental2Controller::OnBallTick);
    mBallTimer.Start();

    window.KeyEventSignal().Connect(this, &TextExperimental2Controller::OnKeyEvent);
  }
  ...
  bool OnBallTick()
  {
    ...
    PrepareFrame(cols, rows);
    SimulateBalls(textWidth, textHeight, cols, rows);
    RasterizeFrame(cols, rows);
    PresentFrame();

    if(mPendingDaliColorAnimationStart)
    {
      mPendingDaliColorAnimationStart = false;
      AttachDaliColorAnimations();
    }
  ...
  void OnKeyEvent(const KeyEvent& event)
  {
    ...
    else if(event.GetKeyName() == "6")
    {
      ...
      DALI_LOG_ERROR("DALI Color Animation %s\n", mDaliColorAnimationEnabled ? "ON" : "OFF");
    }
...
private:
  ...
  float                   mTime                  = 0.0f;
  bool                    mTilesAsyncRendering   = true;
  ...
  bool                    mPendingDaliColorAnimationStart = false;
};
...
int DALI_EXPORT_API main(int argc, char** argv)
{
  ...
  TextExperimental2Controller controller(application);
