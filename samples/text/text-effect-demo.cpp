...
private:
  ...
  void OnInit(Application application)
  {
    ...
    for(auto button : {mBlurToggle, mBlurRadiusButton, mBlurQualityButton})
    {
  ...
  void UpdateBlurControls()
  {
    ...
    mBlurQualityButton.SetEnabled(mBlurEnabled);
    mBlurQualityButton.SetTextColor(UiColor(mBlurEnabled ? CYAN_COLOR : MUTED_TEXT_COLOR));
    mBlurQualityButton.SetBorderlineColor(UiColor(mBlurEnabled ? BLUE_COLOR : CARD_LINE_COLOR));
  }

  void PositionBlurControls(int32_t windowWidth, int32_t windowHeight)
  {
    if(mBlurToggle && mBlurRadiusButton && mBlurQualityButton)
    {
      const float x = std::max(0.0f, static_cast<float>(windowWidth) - 2.0f * BLUR_TOGGLE_WIDTH - BLUR_QUALITY_WIDTH - 2.0f * BLUR_CONTROL_SPACING - BLUR_TOGGLE_MARGIN);
      ...
      mBlurQualityButton.SetRequestedX(x + 2.0f * (BLUR_TOGGLE_WIDTH + BLUR_CONTROL_SPACING));
  ...
  void ApplyTextRenderingMode()
  {
    ...
    for(auto label : {mCards[1].badge, mCards[1].action, mBlurToggle, mBlurRadiusButton, mBlurQualityButton, mLoadingLabel})
    {
  ...
  void TransitionGeneratingStatus(std::size_t index, uint64_t token)
  {
    ...
    StopAnimation(mGeneratingStatusRevealAnimation);
    ConfigureExitReveal(mGeneratingStatus, GetExitBlurRadius(), mBlurQuality);
  ...
  void StartWritingStatus(uint64_t token)
  {
    ...
    ConfigureExitReveal(mDetailStatus, GetExitBlurRadius(), mBlurQuality);
