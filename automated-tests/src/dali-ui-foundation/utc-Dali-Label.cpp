...
int UtcDaliLabelTextRevealBlurValueP(void)
{
  ...
  DALI_TEST_EQUALS(static_cast<int>(reveal.GetBlurQuality()), static_cast<int>(Text::Reveal::BlurQuality::PERFORMANCE), TEST_LOCATION);
  Text::Reveal high(reveal);
  high.SetBlurQuality(Text::Reveal::BlurQuality::HIGH);
  DALI_TEST_CHECK(high != reveal);
  reveal = high;
  ...
  DALI_TEST_EQUALS(static_cast<int>(moved.GetBlurQuality()), static_cast<int>(Text::Reveal::BlurQuality::HIGH), TEST_LOCATION);
  copy = std::move(moved);
  DALI_TEST_EQUALS(copy.GetBlurDurationRatio(), 0.25f, TEST_LOCATION);
  DALI_TEST_EQUALS(static_cast<int>(copy.GetBlurQuality()), static_cast<int>(Text::Reveal::BlurQuality::HIGH), TEST_LOCATION);
